...
detections, objects_path = detector.detectObjectsFromImage(input_image=os.path.join(execution_path , "car.jpg"),
                                                            output_image_path=os.path.join(execution_path , "car_new1.jpg"),
                                                            minimum_percentage_probability=30, extract_detected_objects=True)

for eachObject, eachObjectPath in zip(detections, objects_path): 
  print(eachObject["name"] , " : " , eachObject["percentage_probability"] ) 
  print("Object's image saved in " + eachObjectPath) print("--------------------------------")

#Display the result with bounding boxes

from IPython.display import Image Image("car_new1.jpg")
...
#broccoli, carrot, hot dog, pizza, donot, cake, chair, couch, potted plant, bed,dining table, toilet, tv, laptop,
...
#We can select the objects to be detected by defining the custom objects and we just need to add one more argument
#in the detector.detectCustumObjectsFromImage

custom_objects = detector.CustomObjects(car=True, motorcycle=True) 
detections = detector.detectCustomObjectsFromImage(custom_objects=custom_objects, input_image=os.path.join(execution_path , "image3.jpg"), 
